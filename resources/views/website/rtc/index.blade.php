...
@section('main')
                            ...
                            <tbody>
                                @forelse ($divisions as $division)
                                    <tr>
                                        ...
                                        @if ($showPlanColumns)
                                            @php $terms = ['short','mid','long']; @endphp
                                            @foreach ($terms as $term)
                                                @php
                                                    $rtcForTerm = $rtcs->firstWhere(
                                                        fn($rtc) => $rtc->area_id == $division->id &&
                                                            $rtc->term == $term,
                                                    );
                                ...
                                @empty
                                    <tr>
                                        <td colspan="{{ $showPlanColumns ? 6 : 3 }}" class="text-center text-muted">No data
                                            available</td>
                                    </tr>
                                @endforelse
                            </tbody>
    ...
    {{-- Modal Add hanya ditampilkan untuk non-GM --}}
    @unless ($showPlanColumns)
        ...
        <script>
            ...
            $('#submitPlanBtn').on('click', function() {
                ...
                $.ajax({
                    ...
                    type: 'GET', // saran: ganti ke POST/PUT
                    data: formData,
                    headers: {
