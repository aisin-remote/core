...
@section('main')
    ...
    <div id="kt_app_content_container" class="app-container container-fluid">
        <div class="row">
            ...
            @if (auth()->user()->role === 'User' || 'HRD')
<div class="col-4">
    <div class="card shadow-sm">
        <div class="card-header bg-light text-white">
            <h3 class="card-title mb-0">
                <i class="fas fa-tasks me-2"></i><span class="fw-bold">Checksheet Assessment</span> -
                [{{ count($assessmentTasks) }} Items]
            </h3>
        </div>
        <div class="card-body overflow-auto" style="max-height: 600px;">
            <div class="row g-3">
                @forelse ($assessmentTasks as $task)
                    <div class="col-md-12">
                        <a href="{{ route('checksheet-assessment.index', [
                            'employeeId' => $task['employee_competency_id'],
                            'competencyId' => $task['competency_id'] ?? null
                        ]) }}" 
                        class="text-decoration-none text-dark">
                            <div class="card border-0 shadow-sm bg-warning-subtle hover-shadow">
                                <div class="card-body d-flex justify-content-between align-items-center">
                                    <div>
                                        <h5 class="mb-2">{{ $task['employee_name'] }}</h5>
                                        <p class="mb-0 small">{{ $task['competency_name'] }}</p>
                                    </div>
                                    <span class="badge badge-warning rounded-pill px-3 py-2">
                                        <i class="fas fa-clipboard-list me-2"></i> Need Assessment
                                    </span>
                                </div>
                            </div>
                        </a>
