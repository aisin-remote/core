...
<div class="container">
  <div class="card">
    <div class="card-header">
      <h3>Perbaikan Penilaian - {{ $competency->name }}</h3>
      <div class="alert alert-warning">
        <i class="bi bi-exclamation-triangle me-2"></i>
        Anda sedang memperbaiki penilaian untuk item yang belum mencapai skor "Selalu"
      </div>
    </div>
    <div class="card-body">
      @if($checksheets->count() > 0)
        <form id="assessment-form" method="POST" action="{{ route('checksheet-assessment.store') }}">
          ...
          <div class="checksheet-container">
            ...
            <div class="checksheet-item mb-4 p-4 border rounded border-danger border-2" style="position: relative;">
      ...
      @else
        <div class="alert alert-success">
          ...
          Tidak ada item yang perlu diperbaiki. Semua item sudah mencapai skor "Selalu".
        </div>
        <div class="mt-4">
          <a href="{{ route('employeeCompetencies.index', ['company' => request()->route('company', 'aii')]) }}" class="btn btn-secondary px-4 ms-2">
  ...
  document.getElementById('assessment-form').addEventListener('submit', function(e) {
    ...
    if (invalid.length > 0) {
      Swal.fire({
        icon: 'warning',
        title: 'Oops...',
        html: `Silakan lengkapi penilaian untuk ${invalid.length} item yang perlu diperbaiki!`,
        confirmButtonText: 'OK'
      });
    } else {
      Swal.fire({
        title: 'Konfirmasi Perbaikan',
        text: 'Apakah Anda yakin ingin menyimpan perbaikan penilaian ini?',
        icon: 'question',
