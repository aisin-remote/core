                        ...
                        <td class="text-center">
                            <div class="d-flex flex-column">
                                <button type="button" class="btn btn-sm btn-success btn-approve mb-1"
                                    ...
                                    onclick="confirmApproveArea(this.dataset.area, this.dataset.areaId)">
                                ...
                                <button type="button" class="btn btn-sm btn-danger btn-revise"
                                    data-area="{{ $item['area'] }}"
                                    data-area-id="{{ $item['area_id'] }}"
                                    onclick="confirmReviseArea(this.dataset.area, this.dataset.areaId)">
...
<script>
    ...
    detailModal.addEventListener('show.bs.modal', function(event) {
        ...
        // ====== set URL tombol "View Chart" ======
        const chartBaseUrl = "{{ route('rtc.single-node') }}";
        const chartUrl = chartBaseUrl + `?area=${encodeURIComponent(area)}&area_id=${encodeURIComponent(area_id)}`;
        const chartBtn = document.getElementById('rtcDetailViewChart');
        chartBtn.href = chartUrl;

        // ====== state loading untuk tabel detail ======
        ...
        $.ajax({
            ...
            if (!Array.isArray(res) || res.length === 0) {
                tbody.innerHTML = `
                        <tr>
                            <td colspan="5" class="text-center text-muted py-5">No RTC found in this area.</td>
                        </tr>`;
            } else {
