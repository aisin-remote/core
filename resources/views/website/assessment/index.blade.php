...
@push('scripts')
    ...
    <script>
        // (opsional) pastikan CSRF ikut ke request AJAX
        $.ajaxSetup({
            headers: {
        ...
        // pakai delegation agar handler selalu aktif
        $(document).on('submit', '#assessmentForm', async function(e) {
            e.preventDefault();

            console.group('[Assessment] Submit');
            const $form = $(this);
            ...
            try {
                ...
                console.info('→ assessment_id :', assessment_id || '(baru)');

                // validasi ukuran file (log detail)
                const fileInput = document.getElementById('upload');
                if (fileInput && fileInput.files.length > 0) {
                    const file = fileInput.files[0];
                    console.info('→ file name:', file.name, 'size(bytes):', file.size, 'type:', file.type);
                    if (file.size > 2 * 1024 * 1024) {
                        console.warn('✖ file > 2MB, batalkan submit');
                        await Swal.fire({
                            title: 'Gagal!',
                    ...
                    console.info('→ tidak ada file diupload');
                }

                // siapkan FormData
                const formData = new FormData(this);
                if (assessment_id) formData.append('_method', 'PUT');

                // log isi FormData (hati-hati jika ada file)
                ...
                // kirim AJAX
                console.time('[Assessment] AJAX duration');
                $.ajax({
                        url,
                        type: method,
                        data: formData,
                    ...
                    .done(function(response) {
                        console.timeEnd('[Assessment] AJAX duration');
                        ...
                        Swal.fire({
                            ...
                            // SUKSES → boleh reload (permintaanmu: error jangan reload; sukses tetap reload)
                    ...
                    .fail(function(xhr, status, error) {
                        console.timeEnd('[Assessment] AJAX duration');
                        console.groupCollapsed('✖ fail debug');
                        ...
                        if (xhr.status === 422 && xhr.responseJSON && xhr.responseJSON.errors) {
                            ...
                            msg = errs[firstField][0] || msg;

                            // highlight field yang bermasalah (jika ada id/name yang cocok)
                            try {
                                ...
                                console.warn('gagal highlight invalid fields', e);
                            }

                            console.warn('422 validation errors:', errs);
                        }
                    ...
                    .always(function() {
                        // re-enable tombol & spinner dimatikan
                ...
                console.error('✖ exception di blok submit:', err);
                Swal.fire({
                    title: 'Gagal!',
